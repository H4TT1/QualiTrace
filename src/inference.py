import torch
import cv2
import numpy as np
from model import AnomalyAE
import matplotlib.pyplot as plt

def generate_heatmap(model_path, image_path):
    model = AnomalyAE.load_from_checkpoint(model_path).eval()
    
    # Load and preprocess image
    img = cv2.imread(image_path)
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    img_resized = cv2.resize(img, (256, 256))
    input_tensor = torch.from_numpy(img_resized).permute(2, 0, 1).float().unsqueeze(0) / 255.0

    # Inference
    with torch.no_grad():
        reconstruction = model(input_tensor)
    
    # Calculate Residual (Error) Map
    # We take the difference between original and reconstruction
    residual = torch.abs(input_tensor - reconstruction).squeeze().permute(1, 2, 0).numpy()
    residual_gray = np.mean(residual, axis=2) # Average across channels
    
    # Normalize for visualization
    residual_norm = (residual_gray - residual_gray.min()) / (residual_gray.max() - residual_gray.min())
    heatmap = cv2.applyColorMap((residual_norm * 255).astype(np.uint8), cv2.COLORMAP_JET)

    # Plot
    fig, axes = plt.subplots(1, 3, figsize=(15, 5))
    axes[0].imshow(img_resized)
    axes[0].set_title("Original")
    axes[1].imshow(reconstruction.squeeze().permute(1, 2, 0).numpy())
    axes[1].set_title("Reconstruction")
    axes[2].imshow(heatmap)
    axes[2].set_title("Anomaly Heatmap")
    plt.show()

if __name__ == "__main__":
    # Test it on a 'broken' bottle image
    generate_heatmap("models/last.ckpt", "data/mvtec/bottle/test/broken_large/000.png")